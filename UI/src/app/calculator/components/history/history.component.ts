...
@Component({
  ...
  <div *ngFor="let hist of history" class="mt-2">
    <app-button [value]="getLabel(hist)"  (onClickEvent)="selectHistoricCalculation(hist)" [cssClass]="'w-100 btn btn-light waves-effect'">{{getLabel(hist)}}</app-button>
  </div>
</div>`
})
export class HistoryComponent {
  ...
  {
      return calculation.FirstNumber + " " + calculation.Operator + " " + calculation.SecondNumber + "= " + calculation.Answer;
  }
}
